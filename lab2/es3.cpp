...
/*
  ...
  Parameters:
	...
	- alpha         -> alpha value
*/
void computeForeground(const std::vector<cv::Mat> prevKFrames, const cv::Mat &currI, cv::Mat &out, int th, float alpha)
{
...
int main(int argc, char **argv)
{
	...
	std::cout << "Method: exponential running average" << std::endl;
	...
	while (!exit_loop)
	{
		...
		if (lastKFrames.size() == args.k)
			computeForeground(lastKFrames, image, foreground, args.threshold, args.alpha);
		...
		case 'a':
			...
			{
				args.alpha -= 0.1;
				std::cout << "New alpha value: " << args.alpha << std::endl;
			}
			break;
		case 'A':
			if (args.alpha < 1)
...
bool ParseInputs(ArgumentList &args, int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hi:t:k:a:f:")) != -1)
		switch (c)
		{
		case 't':
		...
		default:
			...
			std::cout << "To increment alpha value:  type A" << std::endl;
			std::cout << "To decrement alpha value:  type a" << std::endl;

			std::cout << "Allowed options:" << std::endl
					  << "   -h                       produce help message" << std::endl
					  << "   -i arg                   image name. Use %0xd format for multiple images." << std::endl
					  << "   -k arg                   number of considered previous frame [default = 5]" << std::endl
					  << "   -a arg                   alpha value (between 0 and 1) [default = 0.5]" << std::endl

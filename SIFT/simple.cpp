...
int main(int argc, char **argv)
{
	...
	while (!exit_loop)
	{
		...
		std::transform(corners.begin(), corners.end(), std::back_inserter(harris_keypoints), [](const cv::Point2f &p)
					   { return cv::KeyPoint(p.x, p.y, 3.0); }); // applica funzione a range vector e memorizza in altro range 3->size del keypoint
